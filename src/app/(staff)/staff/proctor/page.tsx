...
export default function StaffProctorPage() {
  const router = useRouter();
  const [eventId, setEventId] = useState<string | null>(null);
  const [authenticated, setAuthenticated] = useState(false);
  const [stationLabel, setStationLabel] = useState("Dorm Proctor");
  ...
  useEffect(() => {
    ...
    if (storedRole === "proctor" && storedEvent) {
      setEventId(storedEvent);
      setStationLabel(storedLabel || "Dorm Proctor");
      setAuthenticated(true);
      setLoading(false);
      return;
    }

    if (storedRole && storedRole !== "proctor") {
  ...
  function handleAuthenticated(role: StaffRole, label: string | null) {
    ...
    setStationLabel(label || "Dorm Proctor");
    setAuthenticated(true);
  }
