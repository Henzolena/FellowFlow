...
export default function StaffMealsPage() {
  ...
  const [stationLabel, setStationLabel] = useState("Meals Station");
  ...
  useEffect(() => {
    ...
    if (storedRole === "meals" && storedEvent) {
      setEventId(storedEvent);
      setStationLabel(storedLabel || "Meals Station");
    ...
    if (storedRole && storedRole !== "meals") {
      router.replace(`/staff/${storedRole}`);
      return;
    }

    // Need authentication — fetch event
  ...
  function handleAuthenticated(role: StaffRole, label: string | null) {
    ...
    setStationLabel(label || "Meals Station");
    setAuthenticated(true);
  }
