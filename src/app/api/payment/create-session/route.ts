...
async function handleSoloPayment(
  ...
  const eventDates = registration.events.start_date && registration.events.end_date 
    ? `${new Date(registration.events.start_date).toLocaleDateString('en-US', { 
        weekday: 'short', 
        month: 'short', 
        day: 'numeric' 
      })} - ${new Date(registration.events.end_date).toLocaleDateString('en-US', { 
        weekday: 'short', 
        month: 'short', 
        day: 'numeric' 
      })}`
  ...
  // Create multiple line items for better display in Stripe Checkout
  const baseRegistrationItem = {
    price_data: {
      currency: "usd",
      product_data: { 
        name: `${registration.events.name} - Registration`,
        description: `${recomputed.explanationDetail}`
      },
    ...
    metadata: {
      ...
      stripe_product_id: regProduct || 'dynamic',
      item_type: 'registration'
    }
  };
  
  lineItems.push(baseRegistrationItem);
  ...
  if (recomputed.surcharge > 0) {
    ...
    const surchargeDescription = `${recomputed.surchargeLabel || "Late Registration Fee"} for ${registration.first_name} ${registration.last_name}`;
  
  lineItems.push({
      price_data: {
        currency: "usd",
        product_data: { 
          name: recomputed.surchargeLabel || "Late Registration Fee", 
...
async function handleGroupPayment(
    ...
    if (mealIds && mealIds.length > 0) {
      ...
      if (pricePerMeal > 0) {
        ...
        for (const serviceId of mealIds) {
          ...
          const mealDate = service?.meal_date 
            ? new Date(service.meal_date).toLocaleDateString('en-US', { 
                weekday: 'short', 
                month: 'short', 
                day: 'numeric' 
              })
          ...
          const mealDescription = `${service?.meal_name || 'Meal'} for ${r.first_name} ${r.last_name} on ${mealDate}`;
          
          mealLineItems.push({
            price_data: {
              currency: "usd",
              product_data: {
                name: `${service?.meal_name || 'Meal'} - ${r.first_name}`,
  ...
  for (let i = 0; i < registrations.length; i++) {
    ...
    const groupDescription = `${item.explanationDetail || `Registration for ${eventData.name}`} - ${attendeeInfo}`;
    
    lineItems.push({
      price_data: {
        currency: "usd",
        product_data: { 
          name: `${eventData.name} - ${r.first_name} ${r.last_name}`,
  ...
  if (surcharge > 0) {
    ...
    const groupSurchargeDescription = `${surchargeLabel || "Late Registration Fee"} for group of ${registrations.length} registrants`;
    
    lineItems.push({
      price_data: {
        currency: "usd",
        product_data: { 
          name: `${surchargeLabel || "Late Registration Fee"} (Group)`, 
          description: groupSurchargeDescription
        },
